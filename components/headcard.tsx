...
export default function Headcard({name, image, position, github} : { name? : string, image? : string, position? : string, github? : string }) {
    ...
    <div className="text-black flex flex-col items-center">
        <div className="relative w-[clamp(12rem,20vw,15rem)] h-[clamp(16rem,26.5vw,20rem)] bg-[#FFFFFD] border-b-6 border-r-4 overflow-hidden rounded-lg">
          <Image
            src={imgSrc || "/fallback.png"}
            alt={name || "N/A"}
            fill
            sizes="(max-width: 640px) 12rem, (max-width: 1024px) 15vw, 15rem"
      ...
      <p className="text-center text-[clamp(0.5rem,4vw,1.25rem)] font-bold mt-2 w-[15rem]">{name || "N/A"}</p>
      <p className="text-center text-[clamp(0.5rem,4vw,1.25rem)] w-[15rem]">{position || "N/A"}</p>
